...
class HtmlLiveView {
  ...
  _renderContent(html) {
    ...
    if (this._isDark !== undefined && atom.config.get('html-live-view.followTheme')) {
      const theme = this._isDark ? 'dark' : 'light';
      const bg = this._isDark ? '#1e1e1e' : '#ffffff';
      const fg = this._isDark ? '#d4d4d4' : '#333333';
      if (/<html[^>]*>/i.test(processed)) {
        processed = processed.replace(
          /<html([^>]*)>/i,
          '<html$1 data-theme="' + theme + '">' +
          '<style>:root{--host-theme:' + theme + ';--host-bg:' + bg + ';--host-text:' + fg + ';}</style>'
        );
      }
    }
  ...
  updateTheme(isDark) {
    this._isDark = isDark;
    if (this._content) this._renderContent(this._content);
  }
...
module.exports = {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'html-live-view:toggle': () => this.toggle(),
    }));
  ...
  _initThemeAdapter() {
    this._isDark = this._detectDarkTheme();
    this._updateAllViewsTheme();
    this.subscriptions.add(
  ...
  _updateAllViewsTheme() {
    atom.workspace.getPaneItems().forEach((item) => {
      if (item instanceof HtmlLiveView) item.updateTheme(this._isDark);
    });
  },
};
